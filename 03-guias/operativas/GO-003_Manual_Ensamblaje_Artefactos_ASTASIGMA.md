...
### `purpose` (antes `propositoYValorPublico`)
...
### `provided_capabilities` (antes `capacidadesOfrecidas`)
...
### `dependencies` (antes `dependenciasInternas`)
...
### `delegated_role` (antes `rol_delegado`)
...
### `hitl_checkpoints` (antes `puntos_control_hitl`) (Humano-en-el-Bucle)
...
### `description` (dentro de `impactsSystemOfWork`)
...
### `definition_of_done` (alias `definitionOfDone`)
